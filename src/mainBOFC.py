...
def run(pathImages,method,keypnt,numpatch,equalnum,imdes,imsample,percentage,codebook,dist,size,fselec,fselec_perc,histnorm,clust,K,pca,rep,regionscolor,sizecolor,nclusters):
   ...
   name_results_file = 'BOFC_' + im_dataset_name + '_' + keypnt + '_' + str(numpatch) + '_' + str(equalnum) + '_' + imdes + '_' + 'Colors:' + str(regionscolor) + '_' + imsample + '_' + codebook + '_' + str(size) + '_' + 'SizeColors:' + str(sizecolor) + '_' + fselec + '_' + histnorm + '_' + clust + '_'+ dist + '_' + date_time
   ...
   for im in imList:
	 ...
	 for i in range(0,div):
	    for j in range(0,div):
	       ...
	       des_color.append([max_color_l[0], max_color_a[0], max_color_b[0]]) 	 
	 ...
	 number_of_kp_color.append(div*div)
	    ...
	    des_vector_color = np.concatenate((des_vector_color,des_color),axis=0)
         counter += 1   
   ...
   for iteraction in range(0,rep):
      ...
      ceters_color, projections_color = codebook_kmeans_color.obtainCodebook(des_vector_color,des_vector_color)
      ...
      hist_color = histogram.computeHist(projections_color, sizecolor, n_images, number_of_kp_color)
